...
export default function SignupPage() {
  ...
  const handleSendSignupOTP = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Include signup_mode=true to check for existing users
      const { data, error } = await sendSimplifiedWhatsAppOTP(phoneNumber, false, true)
      console.log('Send OTP response:', data, error)

      if (error) {
        // Special case for user already exists
        if (error.redirect_to_login) {
          setError('A user with this phone number already exists. Please log in instead.')
          // Add a redirect button that will be shown alongside the error
          setTimeout(() => {
            navigate('/login')
      ...
      if (data?.rate_limited) {
        ...
        if (data?.debug?.testCode) {
          ...
          console.log('Test code received in signup page:', data.debug.testCode);
        } else {
        ...
        const { seconds_remaining, request_count } = data.rate_limit;
        if (seconds_remaining > 0) {
          console.log(`Rate limit info: ${seconds_remaining}s remaining, ${request_count} requests`);
        }
      }
  ...
  const handleOTPVerified = async (data: any) => {
    ...
    if (data?.is_new_user !== undefined) {
      sessionStorage.setItem('is_new_user', data.is_new_user.toString())
    }
        ...
        <div className="w-full max-w-md space-y-8 relative z-10">
          ...
          <div className="bg-[var(--nenrin-mist)] p-4 rounded-lg border border-[var(--nenrin-forest)]/20">
            <div className="flex items-center space-x-3 mb-2">
              ...
              <h3 className="font-medium text-[var(--nenrin-ink)]">WhatsApp Authentication</h3>
            </div>
            <p className="text-sm text-[var(--nenrin-sage)]">
              We use WhatsApp to verify your identity and keep your account secure. You'll receive a verification code via WhatsApp.
            </p>
          ...
          <div className="text-center">
            <p className="text-sm text-[var(--nenrin-sage)]">
              ...
              <Link to="/login" className="font-medium text-[var(--nenrin-forest)] hover:text-[var(--forest-hover)]">
                Sign in
              </Link>
            </p>
          </div>
