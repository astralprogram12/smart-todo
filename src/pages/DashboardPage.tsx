...
/**
 * Nenrin — First Dashboard (Full Version with Functional Logout)
 * ------------------------------------------------------------------
 * - Restores all original animations, components, and content.
 * - Includes a simple, functional logout button in the header.
 * - Redirects to a specified external URL on logout.
 */
...
    }
    ...
    {/* ✅ LOGOUT BUTTON STYLE ADDED */}
    .btn-logout {
      background-color: rgba(26, 74, 53, 0.05);
      color: var(--nenrin-forest);
      border: 1px solid rgba(26, 74, 53, 0.1);
      width: 40px;
      height: 40px;
    ...
    .btn-logout:hover {
      background-color: rgba(26, 74, 53, 0.1);
      border-color: rgba(26, 74, 53, 0.2);
...
  Check: (props: IconProps) => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M20 6L9 17l-5-5"/></svg>),
  // ✅ LOGOUT ICON ADDED
  Logout: (props: IconProps) => (<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" {...props}><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"></path><polyline points="16 17 21 12 16 7"></polyline><line x1="21" y1="12" x2="9" y2="12"></line></svg>),
};
...
// ----------------- Modal -----------------
function SimilarExpressionsModal({ isOpen, onClose, suggestion }: any) {
...
            {suggestion.similar.map((phrase: string, index: number) => (
              <li key={index}>
                <a 
                  href={waLink(phrase)} 
                  target="_blank" 
...
    const openModal = (suggestion: any) => {
...
function NenrinFirstDashboard(){
    ...
    /**
     * ✅ LOGOUT FUNCTION
     * Clears all items from localStorage (like session tokens) and redirects the user.
     */
    const handleLogout = () => {
        console.log("Logging out and redirecting...");
        localStorage.clear(); // Wipes all session data from the browser
        window.location.href = 'https://v0-gemini-todo-app-nine.vercel.app/'; // Redirects to the specified URL
    };
...
            </span>
            {/* ✅ ADDED LOGOUT BUTTON WITH ONCLICK HANDLER */}
            <button 
